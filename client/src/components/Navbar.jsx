            ...
            <header className="flex flex-wrap sm:justify-start sm:flex-col z-50 w-full bg-[#1d1b44] border-b border-gray-200 text-sm pb-2 sm:pb-0 dark:border-gray-700">
                <div className="max-w-[85rem] mx-auto w-full px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-end gap-x-5 w-full py-2 sm:pt-2 sm:pb-0">
                        ...
                        {userLoggedIn ? (
                            <button onClick={handleLogout} className="inline-flex justify-center cursor-pointer items-center gap-2 font-medium text-slate-600 hover:text-slate-500 text-sm dark:text-slate-400 dark:hover:text-slate-300" href="#">
                                Logout
                            </button>
                        ) : (
                            <>
                                <a onClick={() => navigate('/login')} className="inline-flex justify-center cursor-pointer items-center gap-2 font-medium text-slate-600 hover:text-slate-500 text-sm dark:text-slate-400 dark:hover:text-slate-300" href="#">
                                    Sign In
                                </a>
                                <a className="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold cursor-pointer rounded-lg border border-blue-600 text-blue-600 hover:border-blue-500 hover:text-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:border-blue-500 dark:text-blue-500 dark:hover:text-blue-400 dark:hover:border-blue-400 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" onClick={() => navigate('/signup')}>
                                    Get started
                                </a>
                            </>
                        )}
                ...
                <nav className="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
                    <div className="flex items-center justify-between">
                        <a onClick={()=>navigate('/')} className="flex-none text-2xl font-semibold cursor-pointer dark:text-white" aria-label="Brand">SOCIALHUB</a>
                        ...
                        <div className="flex flex-col gap-y-4 gap-x-0 text-lg mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
                            ...
                            <div className="hs-dropdown [--strategy:static] sm:[--strategy:fixed] [--adaptive:none] sm:[--trigger:hover] sm:py-4">
                                ...
                                <div className="hs-dropdown-menu transition-[opacity,margin] duration-[0.1ms] sm:duration-[150ms] hs-dropdown-open:opacity-100 opacity-0 sm:w-48 hidden z-10 bg-white sm:shadow-md rounded-lg p-2 dark:bg-gray-800 sm:dark:border dark:border-gray-700 dark:divide-gray-700 before:absolute top-full sm:border before:-top-5 before:start-0 before:w-full before:h-5 ">
                                    <a onClick={()=>navigate('/about-us')} className="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-lg text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600 cursor-pointer" >
                                        About
                                    </a>
                                    <a onClick={handleMyAccount} className="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-lg text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600 cursor-pointer" >
                                        My Account
                                    </a>
                                    {userLoggedIn ? (
                                        <button onClick={handleLogout} className='flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600'>
                                            Logout
                                        </button>
                                    ) : null}
                                </div>
